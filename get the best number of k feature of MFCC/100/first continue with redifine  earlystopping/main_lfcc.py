...
# Copy the LFCC class and helper functions here
...
class LFCC(torch.nn.Module):
    """Create the linear-frequency cepstral coefficients (LFCC features) from an audio signal.
...
# Function to extract LFCC features for a given folder
def extract_lfcc_features(folder_path, lfcc_extractor):
    lfcc_features = []
    ...
    for file_name in file_list:
        ...
        # Load audio file with explicit format specification
        waveform, sample_rate = torchaudio.load(file_path, format='wav')

        # Extract LFCC features
        lfcc = lfcc_extractor(waveform)

        # Append LFCC features to the list
        lfcc_features.append(lfcc)

    return lfcc_features

# Set the paths to the real and fake folders
fake_folder_path= 'C:\\Users\\aa\Desktop\\generated_audio\\generated_audio\\data set all\\fake1'
real_folder_path = 'C:\\Users\aa\\Desktop\\generated_audio\\generated_audio\\data set all\\real1'

# Create an instance of the LFCC class
lfcc_extractor = LFCC(sample_rate=16000, n_lin=128, n_lfcc=40, f_min=0.0)

# Extract LFCC features for real and fake audio data
real_lfcc_features = extract_lfcc_features(real_folder_path, lfcc_extractor)
fake_lfcc_features = extract_lfcc_features(fake_folder_path, lfcc_extractor)

# Display the shape of the extracted LFCC features for the first file
print("Real LFCC features shape:", real_lfcc_features[0].shape)
print("Fake LFCC features shape:", fake_lfcc_features[0].shape)
